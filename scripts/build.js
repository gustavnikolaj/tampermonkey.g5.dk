...
function main(srcDir, templateDir, outputDir) {
    ...
    const entryTemplate = fs.readFileSync(path.resolve(templateDir, 'script-entry.html'), 'utf-8')
    const indexTemplate = fs.readFileSync(path.resolve(templateDir, 'index.html'), 'utf-8')

    const entries = scripts.map(x => ({
        href: x.href,
    ...
    const indexContent = indexTemplate.replace('%%ENTRIES%%', '\n' + entries.join('\n'))
